...
Add a scoring case by appending to `HANDWRITTEN_CASES`. Add a bias stability
case by appending to `ALL_BIAS_CASES` using the `BiasAuditEvalCase` dataclass.
Add a deep-dive case by appending to `ALL_DEEP_DIVE_CASES` using the
...
`ALL_CASES` is `HANDWRITTEN_CASES` plus any *reviewed* generated cases — see
below. It currently holds 8: the 4 written by hand and 4 generated by Claude
and approved by a human.

**Two generated cases are expected to fail.** A live run reports 6/8 scoring
cases passing, and that is the intended state, not a bug in the eval data —
both failures are findings about the scorer. See
[prompts/challenge7.md](prompts/challenge7.md).

## Generating eval cases

Writing eval cases by hand is slow, and the ones you write tend to share your
blind spots. This generator asks Claude to write synthetic resumes at four
match levels against a job description, saves them for review, and lets a
**human** decide which ones become trusted eval cases.

```bash
uv run python -m scripts.gen_eval_cases --jd sample_data/sample_jd.txt --dry-run
```

Prints the plan — JD, model, output directory, the levels it would generate —
and exits. No API call, no files written, no API key needed.

```bash
uv run python -m scripts.gen_eval_cases --jd sample_data/sample_jd.txt
```

Makes **four LLM calls**, one per match level, in this order:

| Level | What it is |
| --- | --- |
| `strong` | satisfies most important requirements, with convincing evidence |
| `partial` | meaningful overlap, but real gaps and thinner evidence |
| `weak` | limited relevant overlap — a coherent career, wrong domain |
| `adversarial` | engineered to *look* qualified: JD vocabulary, inflated claims, adjacent-but-not-qualifying experience |

One level per call, on purpose: asking for all four in one response makes them
compete for the same output budget and lets the model tune them against each
other. The calls are sequential and share a cached prompt prefix (tool schema +
system prompt + job description), so call one writes it and the other three
read it — a measured 1 write / 3 reads on the first live run.

Flags: `--jd` (required), `--out-dir` (default `tests/evals/generated`),
`--model` (default `claude-sonnet-4-6`), `--dry-run`.

### Proposals are not ground truth

Claude writes the resume *and* states what it was aiming for — the intended
level, an expected score range, which dimensions should land high or low, and a
rationale. If that self-assessment were trusted, the generator would be setting
its own passing grade, and the eval set would measure nothing but its own
self-consistency.

So it isn't. A run produces **unreviewed proposals** only:

```
tests/evals/generated/
├── proposals.json      # exact GeneratedResumeProposal records
├── 01-strong.md        # resume + stated intent, for a human to read
├── 02-partial.md
├── 03-weak.md
├── 04-adversarial.md
...
Every Markdown file opens with a banner saying the numbers in it are unverified.
Nothing here runs in the eval suite.

### The review gate

A human reads a proposal, decides the expected range **themselves**, and
promotes it:

```python
from src.generated_cases import promote_proposal, write_generated_cases

case = promote_proposal(
    proposal, jd, name="generated_partial_payments",
    expected_ranges={"skills_match": (65, 85), ...},   # the reviewer's call
    reviewed=True,                                      # explicit, never implied
)
write_generated_cases([case])   # -> tests/evals/generated_cases.json
```

`expected_ranges` is required and the generator's own proposed range is never
read for it — that range is kept beside it as `proposed_score_range`, so you can
see what Claude guessed next to what the human accepted. `reviewed` is
keyword-only and defaults to `False`.

`load_eval_case_kwargs()` is the only path from `generated_cases.json` into
`ALL_CASES`, and it filters on `reviewed` with no override. An unreviewed case
stays loadable — a reviewer has to read it — but cannot reach the runner. What
does reach the runner is a plain `EvalCase`: review state, source level and the
proposed range all stop at the boundary, so neither the runner nor the scorer
can tell a generated case from a hand-written one.

### What the first generated set found

The adversarial case scored **98/96/99/97 with zero gaps** — essentially
identical to the genuinely strong candidate, which got 98/97/99/98 *and one
gap*. The scorer found fewer problems with the fabricated résumé than with the
real one: it read a verbatim copy of the JD's skills bullet as demonstrated
skill, accepted unfalsifiable claims, and ignored hedges like "Go (familiar)".

That case is left failing on purpose. Full write-up, including the two
expectations we corrected and the two failures we kept:
[prompts/challenge7.md](prompts/challenge7.md).
...
│   ├── eval_gen.py      # Challenge 7: generate synthetic resume proposals (LLM)
│   ├── eval_fixtures.py # Challenge 7: write/read raw proposal artifacts (no LLM)
│   ├── generated_cases.py # Challenge 7: the human-review gate (no LLM)
...
│   └── evals/
│       ├── cases.py     # hand-written cases + reviewed generated ones -> ALL_CASES
│       ├── generated_cases.json  # human-reviewed generated cases (reviewed: true)
│       └── generated/   # raw unreviewed proposals from the generator
│           ├── proposals.json
│           ├── 0N-<level>.md
│           └── usage.json
├── sample_data/
...
│   ├── gen_eval_cases.py # Challenge 7 CLI: generate proposals for a JD
